/**
  ******************************************************************************
  * @file    vl53l8cx_api_cf.c
  * @brief   C-only, Crazyflie-compatible implementation for VL53L8CX API parts
  *
  * Notes:
  *  - No mbed/C++ headers used.
  *  - All delays and bus I/O are done via platform.c (VL53L8CX_WaitMs, Rd/Wr).
  *  - Keep ST’s function signatures to remain ULD-compatible.
  *
  * Copyright (c) 2021 STMicroelectronics.
  * Additional Crazyflie adaptation (c) 2025, provided AS-IS.
  ******************************************************************************
  */
...
#include "platform.h"  /* provides VL53L8CX_Platform + low-level bus & wait */
// === Redirect VL53L8CX ULD allocations to the static arena ===
...
/**
 * @brief Polls for embedded MCU boot.
 */
static uint8_t _vl53l8cx_poll_for_mcu_boot(VL53L8CX_Configuration *p_dev)
{
  ...
  do {
    ...
    if ((go2_status0 & (uint8_t)0x01) != (uint8_t)0) {
      /* explicit error bit */
      break;
    }
...
/**
 * @brief Loads NVM-based offset data (with optional 4x4 extrapolation) to sensor.
 */
    ...
    uint8_t                 resolution)
{
  ...
  /* Extrapolate from 8x8 to 4x4 if needed */
  ...
  status |= VL53L8CX_WrMulti(&p_dev->platform, 0x2e18,
...
/**
 * @brief Loads Xtalk data (with optional 4x4 extrapolation) to sensor.
 */
    ...
    uint8_t                 resolution)
{
  ...
  (void)memcpy(p_dev->temp_buffer, p_dev->xtalk_data, VL53L8CX_XTALK_BUFFER_SIZE);

  if (resolution == (uint8_t)VL53L8CX_RESOLUTION_4X4) {
    (void)memcpy(&p_dev->temp_buffer[0x08],  res4x4,    sizeof(res4x4));
...
uint8_t vl53l8cx_is_alive(VL53L8CX_Configuration *p_dev, uint8_t *p_is_alive)
{
  ...
  status |= VL53L8CX_RdByte(&p_dev->platform, 0x0000, &device_id);
  status |= VL53L8CX_RdByte(&p_dev->platform, 0x0001, &revision_id);
  status |= VL53L8CX_WrByte(&p_dev->platform, 0x7fff, 0x02);

  *p_is_alive = (uint8_t)((device_id == (uint8_t)0xF0) && (revision_id == (uint8_t)0x0C));
  return status;
}
...
uint8_t vl53l8cx_init(VL53L8CX_Configuration *p_dev)
{
  ...
  uint32_t single_range = 0x01U;
  uint32_t crc_checksum = 0x00U;
  ...
  status |= _vl53l8cx_poll_for_answer(p_dev, 1, 0, 0x21, 0xFF, 0x04);

  status = 0; /* preserve ST behavior */
  ...
  status |= VL53L8CX_WrMulti(&p_dev->platform, 0x0000,
                             (uint8_t *)&VL53L8CX_FIRMWARE[0x00000], 0x8000);
  status |= VL53L8CX_WrByte(&p_dev->platform, 0x7fff, 0x0a);
  status |= VL53L8CX_WrMulti(&p_dev->platform, 0x0000,
                             (uint8_t *)&VL53L8CX_FIRMWARE[0x08000], 0x8000);
  ...
  status |= VL53L8CX_RdMulti(&p_dev->platform,
                             (uint16_t)(0x812FFC & 0xFFFF), p_dev->temp_buffer, 4);
  VL53L8CX_SwapBuffer(p_dev->temp_buffer, 4);
  (void)memcpy((uint8_t *)&crc_checksum, &p_dev->temp_buffer[0], 4);
  if (crc_checksum != (uint32_t)0x0C0B6C9E) {
    status |= VL53L8CX_STATUS_FW_CHECKSUM_FAIL;
    goto exit;
  }

  /* Get offset NVM, push to sensor */
  status |= VL53L8CX_WrMulti(&p_dev->platform, 0x2fd8,
                             (uint8_t *)VL53L8CX_GET_NVM_CMD,
                             (uint16_t)sizeof(VL53L8CX_GET_NVM_CMD));
  ...
  status |= VL53L8CX_WrMulti(&p_dev->platform, 0x2c34,
                             ...
                             (uint16_t)sizeof(VL53L8CX_DEFAULT_CONFIGURATION));
...
uint8_t vl53l8cx_set_i2c_address(VL53L8CX_Configuration *p_dev, uint16_t i2c_address)
{
  ...
  status |= VL53L8CX_WrByte(&p_dev->platform, 0x7fff, 0x00);
  status |= VL53L8CX_WrByte(&p_dev->platform, 0x0004, (uint8_t)(i2c_address >> 1));
...
uint8_t vl53l8cx_get_power_mode(VL53L8CX_Configuration *p_dev, uint8_t *p_power_mode)
{
  ...
  status |= VL53L8CX_WrByte(&p_dev->platform, 0x7FFF, 0x00);
  status |= VL53L8CX_RdByte(&p_dev->platform, 0x0009, &tmp);
  ...
  switch (tmp) {
    ...
    case 0x2:
      ...
      *p_power_mode = (tmp == 0x43)
                      ? VL53L8CX_POWER_MODE_DEEP_SLEEP
                      : VL53L8CX_POWER_MODE_SLEEP;
      break;
    default:
      *p_power_mode = 0;
...
{
  uint8_t status = VL53L8CX_STATUS_OK;
  uint8_t current = 0, stored = 0;

  status |= vl53l8cx_get_power_mode(p_dev, &current);
  ...
  switch (power_mode) {
    case VL53L8CX_POWER_MODE_WAKEUP:
      status |= VL53L8CX_WrByte(&p_dev->platform, 0x7FFF, 0x00);
      status |= VL53L8CX_WrByte(&p_dev->platform, 0x0009, 0x04);
      status |= VL53L8CX_RdByte(&p_dev->platform, 0x000F, &stored);
      if (stored == 0x43) {
        status |= VL53L8CX_WrByte(&p_dev->platform, 0x000F, 0x40);
      }
    ...
    case VL53L8CX_POWER_MODE_SLEEP:
      status |= VL53L8CX_WrByte(&p_dev->platform, 0x7FFF, 0x00);
      status |= VL53L8CX_WrByte(&p_dev->platform, 0x0009, 0x02);
...
uint8_t vl53l8cx_start_ranging(VL53L8CX_Configuration *p_dev)
{
  ...
  for (i = 0; i < (uint32_t)(sizeof(output) / sizeof(uint32_t)); i++) {
    if ((output[i] == (uint32_t)0) ||
        ((output_bh_enable[i / 32U] & ((uint32_t)1 << (i % 32U))) == (uint32_t)0)) {
      continue;
    }
    ...
    if (((uint8_t)bh_ptr->type >= (uint8_t)0x1) && ((uint8_t)bh_ptr->type < (uint8_t)0x0d)) {
      ...
      p_dev->data_read_size += (uint32_t)bh_ptr->type * (uint32_t)bh_ptr->size;
    } else {
      p_dev->data_read_size += (uint32_t)bh_ptr->size;
    }
    p_dev->data_read_size += (uint32_t)4;
  }
  p_dev->data_read_size += (uint32_t)24;

  status |= vl53l8cx_dci_write_data(p_dev, (uint8_t *)&output,
                                    VL53L8CX_DCI_OUTPUT_LIST,
  ...
  status |= VL53L8CX_WrByte(&p_dev->platform, 0x7fff, 0x00);
  status |= VL53L8CX_WrByte(&p_dev->platform, 0x0009, 0x05);
  status |= VL53L8CX_WrByte(&p_dev->platform, 0x7fff, 0x02);

  /* Start ranging */
  status |= VL53L8CX_WrMulti(&p_dev->platform, (uint16_t)(VL53L8CX_UI_CMD_END - (uint16_t)3),
                             (uint8_t *)cmd, (uint16_t)sizeof(cmd));
  status |= _vl53l8cx_poll_for_answer(p_dev, 4, 1, VL53L8CX_UI_CMD_STATUS, 0xff, 0x03);

  /* Validate ui range data size */
  status |= vl53l8cx_dci_read_data(p_dev, p_dev->temp_buffer, 0x5440, 12);
  ...
  /* Laser safety check */
  status |= vl53l8cx_dci_read_data(p_dev, p_dev->temp_buffer, 0xE0C4, 8);
...
uint8_t vl53l8cx_stop_ranging(VL53L8CX_Configuration *p_dev)
{
  ...
  if ((auto_stop_flag != (uint32_t)0x4FF) && (p_dev->is_auto_stop_enabled == (uint8_t)0)) {
    ...
    /* Provoke MCU stop */
    status |= VL53L8CX_WrByte(&p_dev->platform, 0x0015, 0x16);
    status |= VL53L8CX_WrByte(&p_dev->platform, 0x0014, 0x01);

    /* Poll for stop */
    while (((tmp & (uint8_t)0x80) >> 7) == (uint8_t)0x00) {
      status |= VL53L8CX_RdByte(&p_dev->platform, 0x0006, &tmp);
      status |= VL53L8CX_WaitMs(&p_dev->platform, 10);
      timeout++;

      if (timeout > (uint16_t)500) { /* ~5s */
        status |= tmp;
        break;
      }
    }
  ...
  /* Check GO2 status 1 if still OK */
  status |= VL53L8CX_RdByte(&p_dev->platform, 0x0006, &tmp);
  if ((tmp & (uint8_t)0x80) != (uint8_t)0) {
    status |= VL53L8CX_RdByte(&p_dev->platform, 0x0007, &tmp);
    if ((tmp != (uint8_t)0x84) && (tmp != (uint8_t)0x85)) {
      status |= tmp;
    }
  }

  /* Undo stop and xshut bypass */
  status |= VL53L8CX_WrByte(&p_dev->platform, 0x7fff, 0x00);
  status |= VL53L8CX_WrByte(&p_dev->platform, 0x0014, 0x00);
  status |= VL53L8CX_WrByte(&p_dev->platform, 0x0015, 0x00);
...
uint8_t vl53l8cx_check_data_ready(VL53L8CX_Configuration *p_dev, uint8_t *p_isReady)
{
  ...
  status |= VL53L8CX_RdMulti(&p_dev->platform, 0x0000, p_dev->temp_buffer, 4);

  if ((p_dev->temp_buffer[0] != p_dev->streamcount) &&
      (p_dev->temp_buffer[0] != (uint8_t)255) &&
      (p_dev->temp_buffer[1] == (uint8_t)0x05) &&
      ((p_dev->temp_buffer[2] & (uint8_t)0x05) == (uint8_t)0x05) &&
    ...
    *p_isReady = (uint8_t)1;
    ...
    if ((p_dev->temp_buffer[3] & (uint8_t)0x80) != (uint8_t)0) {
      status |= p_dev->temp_buffer[2]; /* GO2 error */
    }
    *p_isReady = (uint8_t)0;
  }
  return status;
...
uint8_t vl53l8cx_get_ranging_data(VL53L8CX_Configuration *p_dev, VL53L8CX_ResultsData *p_results)
{
  ...
  union Block_header *bh_ptr;
uint32_t i, j, msize;
(void)j;  // silence -Werror=unused-variable without changing behavior

  status |= VL53L8CX_RdMulti(&p_dev->platform, 0x0000, p_dev->temp_buffer, p_dev->data_read_size);
  p_dev->streamcount = p_dev->temp_buffer[0];
  VL53L8CX_SwapBuffer(p_dev->temp_buffer, (uint16_t)p_dev->data_read_size);

  /* Parse blocks; start after 16-byte header */
  for (i = 16; i < (uint32_t)p_dev->data_read_size; i += 4U) {
    bh_ptr = (union Block_header *)&p_dev->temp_buffer[i];
    if ((bh_ptr->type > (uint32_t)0x1) && (bh_ptr->type < (uint32_t)0x0d)) {
      msize = (uint32_t)bh_ptr->type * (uint32_t)bh_ptr->size;
    } else {
      msize = (uint32_t)bh_ptr->size;
    }

    switch (bh_ptr->idx) {
      case VL53L8CX_METADATA_IDX:
        p_results->silicon_temp_degc = (int8_t)p_dev->temp_buffer[i + 12U];
        break;
#ifndef VL53L8CX_DISABLE_AMBIENT_PER_SPAD
      case VL53L8CX_AMBIENT_RATE_IDX:
        (void)memcpy(p_results->ambient_per_spad, &p_dev->temp_buffer[i + 4U], msize);
        break;
#endif
#ifndef VL53L8CX_DISABLE_NB_SPADS_ENABLED
      case VL53L8CX_SPAD_COUNT_IDX:
        (void)memcpy(p_results->nb_spads_enabled, &p_dev->temp_buffer[i + 4U], msize);
        break;
#endif
#ifndef VL53L8CX_DISABLE_NB_TARGET_DETECTED
      case VL53L8CX_NB_TARGET_DETECTED_IDX:
        (void)memcpy(p_results->nb_target_detected, &p_dev->temp_buffer[i + 4U], msize);
        break;
#endif
#ifndef VL53L8CX_DISABLE_SIGNAL_PER_SPAD
      case VL53L8CX_SIGNAL_RATE_IDX:
        (void)memcpy(p_results->signal_per_spad, &p_dev->temp_buffer[i + 4U], msize);
        break;
#endif
#ifndef VL53L8CX_DISABLE_RANGE_SIGMA_MM
      case VL53L8CX_RANGE_SIGMA_MM_IDX:
        (void)memcpy(p_results->range_sigma_mm, &p_dev->temp_buffer[i + 4U], msize);
        break;
#endif
#ifndef VL53L8CX_DISABLE_DISTANCE_MM
      case VL53L8CX_DISTANCE_IDX:
        (void)memcpy(p_results->distance_mm, &p_dev->temp_buffer[i + 4U], msize);
        break;
#endif
#ifndef VL53L8CX_DISABLE_REFLECTANCE_PERCENT
      case VL53L8CX_REFLECTANCE_EST_PC_IDX:
        (void)memcpy(p_results->reflectance, &p_dev->temp_buffer[i + 4U], msize);
        break;
#endif
#ifndef VL53L8CX_DISABLE_TARGET_STATUS
      case VL53L8CX_TARGET_STATUS_IDX:
        (void)memcpy(p_results->target_status, &p_dev->temp_buffer[i + 4U], msize);
        break;
#endif
#ifndef VL53L8CX_DISABLE_MOTION_INDICATOR
      case VL53L8CX_MOTION_DETEC_IDX:
        (void)memcpy(&p_results->motion_indicator, &p_dev->temp_buffer[i + 4U], msize);
        break;
#endif
      default:
        break;
...
#ifndef VL53L8CX_DISABLE_TARGET_STATUS
        p_results->target_status[(VL53L8CX_NB_TARGET_PER_ZONE * i) + j] = (uint8_t)255;
#endif
      }
    }
  }
  ...
  /* Sanity check header/footer IDs */
  header_id  = ((uint16_t)p_dev->temp_buffer[0x8]  << 8) | (uint16_t)p_dev->temp_buffer[0x9];
  footer_id  = ((uint16_t)p_dev->temp_buffer[p_dev->data_read_size - 4] << 8)
             | (uint16_t)p_dev->temp_buffer[p_dev->data_read_size - 3];
...
/* --- Small getters/setters follow (unchanged semantics, C-only includes) --- */
...
uint8_t vl53l8cx_get_resolution(VL53L8CX_Configuration *p_dev, uint8_t *p_resolution)
{
  ...
  *p_resolution = (uint8_t)(p_dev->temp_buffer[0] * p_dev->temp_buffer[1]);
  return status;
}
...
{
  uint8_t status = VL53L8CX_STATUS_OK;
  status |= vl53l8cx_dci_read_data(p_dev, p_dev->temp_buffer, VL53L8CX_DCI_FREQ_HZ, 4);
...
{
  return vl53l8cx_dci_replace_data(p_dev, p_dev->temp_buffer,
...
{
  uint8_t status = VL53L8CX_STATUS_OK;
  status |= vl53l8cx_dci_read_data(p_dev, p_dev->temp_buffer, VL53L8CX_DCI_INT_TIME, 20);
  (void)memcpy(p_time_ms, &p_dev->temp_buffer[0], 4);
...
{
  uint8_t status = VL53L8CX_STATUS_OK;
  uint32_t integ = integration_time_ms;

  if ((integ < (uint32_t)2) || (integ > (uint32_t)1000)) {
    status |= VL53L8CX_STATUS_INVALID_PARAM;
  } else {
    integ *= (uint32_t)1000;
    status |= vl53l8cx_dci_replace_data(p_dev, p_dev->temp_buffer,
                                        VL53L8CX_DCI_INT_TIME, 20,
                                        (uint8_t *)&integ, 4, 0x00);
  }
  return status;
}
...
uint8_t vl53l8cx_get_sharpener_percent(VL53L8CX_Configuration *p_dev, uint8_t *p_sharpener_percent)
{
  ...
  *p_sharpener_percent = (uint8_t)((p_dev->temp_buffer[0xD] * (uint16_t)100) / (uint16_t)255);
  return status;
}
...
uint8_t vl53l8cx_set_sharpener_percent(VL53L8CX_Configuration *p_dev, uint8_t sharpener_percent)
{
  if (sharpener_percent >= (uint8_t)100) {
    return VL53L8CX_STATUS_INVALID_PARAM;
  }
  uint8_t sharp = (uint8_t)((sharpener_percent * (uint16_t)255) / (uint16_t)100);
  return vl53l8cx_dci_replace_data(p_dev, p_dev->temp_buffer,
                                   VL53L8CX_DCI_SHARPENER, 16,
                                   (uint8_t *)&sharp, 1, 0x0D);
}

uint8_t vl53l8cx_get_target_order(VL53L8CX_Configuration *p_dev, uint8_t *p_target_order)
{
  uint8_t status = VL53L8CX_STATUS_OK;
  status |= vl53l8cx_dci_read_data(p_dev, p_dev->temp_buffer, VL53L8CX_DCI_TARGET_ORDER, 4);
  *p_target_order = p_dev->temp_buffer[0x0];
  return status;
}

uint8_t vl53l8cx_set_target_order(VL53L8CX_Configuration *p_dev, uint8_t target_order)
{
  if ((target_order != VL53L8CX_TARGET_ORDER_CLOSEST) &&
      (target_order != VL53L8CX_TARGET_ORDER_STRONGEST)) {
    return VL53L8CX_STATUS_INVALID_PARAM;
  }
  return vl53l8cx_dci_replace_data(p_dev, p_dev->temp_buffer,
...
uint8_t vl53l8cx_get_ranging_mode(VL53L8CX_Configuration *p_dev, uint8_t *p_ranging_mode)
{
  ...
  *p_ranging_mode = (p_dev->temp_buffer[0x01] == (uint8_t)0x1)
...
uint8_t vl53l8cx_set_ranging_mode(VL53L8CX_Configuration *p_dev, uint8_t ranging_mode)
{
  ...
  status |= vl53l8cx_dci_write_data(p_dev, p_dev->temp_buffer, VL53L8CX_DCI_RANGING_MODE, 8);
  status |= vl53l8cx_dci_write_data(p_dev, (uint8_t *)&single_range,
                                    VL53L8CX_DCI_SINGLE_RANGE,
                                    (uint16_t)sizeof(single_range));
  return status;
...
uint8_t vl53l8cx_get_external_sync_pin_enable(VL53L8CX_Configuration *p_dev,
                                              uint8_t *p_is_sync_pin_enabled)
{
  uint8_t status = VL53L8CX_STATUS_OK;
  status |= vl53l8cx_dci_read_data(p_dev, p_dev->temp_buffer, VL53L8CX_DCI_SYNC_PIN, 4);

  *p_is_sync_pin_enabled = ((p_dev->temp_buffer[3] & (uint8_t)0x2) != 0) ? (uint8_t)1 : (uint8_t)0;
  return status;
}
...
uint8_t vl53l8cx_get_VHV_repeat_count(VL53L8CX_Configuration *p_dev, uint32_t *p_repeat_count)
{
  ...
  status |= vl53l8cx_dci_read_data(p_dev, p_dev->temp_buffer, VL53L8CX_DCI_VHV_CONFIG, 16);
...
uint8_t vl53l8cx_dci_read_data(VL53L8CX_Configuration *p_dev,
                               uint8_t *data, uint32_t index, uint16_t data_size)
{
  int16_t i;
  uint8_t status = VL53L8CX_STATUS_OK;
  ...
  cmd[1] = (uint8_t)(index & 0xffU);
  cmd[2] = (uint8_t)((data_size & (uint16_t)0xff0) >> 4);
  cmd[3] = (uint8_t)((data_size & (uint16_t)0x00f) << 4);

  status |= VL53L8CX_WrMulti(&p_dev->platform,
                             (uint16_t)(VL53L8CX_UI_CMD_END - 11U),
                             cmd, (uint16_t)sizeof(cmd));
  status |= _vl53l8cx_poll_for_answer(p_dev, 4, 1, VL53L8CX_UI_CMD_STATUS, 0xff, 0x03);

  status |= VL53L8CX_RdMulti(&p_dev->platform, VL53L8CX_UI_CMD_START,
                             p_dev->temp_buffer, rd_size);
  VL53L8CX_SwapBuffer(p_dev->temp_buffer, (uint16_t)(data_size + 12U));
...
uint8_t vl53l8cx_dci_write_data(VL53L8CX_Configuration *p_dev,
                                uint8_t *data, uint32_t index, uint16_t data_size)
{
  uint8_t status = VL53L8CX_STATUS_OK;
  int16_t i;
  ...
  uint8_t footer[]  = {0x00, 0x00, 0x00, 0x0f, 0x05, 0x01,
                       (uint8_t)(((uint16_t)(data_size + 8)) >> 8),
                       (uint8_t)(((uint16_t)(data_size + 8)) & 0xFF)};

  uint16_t address = (uint16_t)(VL53L8CX_UI_CMD_END - (data_size + (uint16_t)12) + 1U);
  ...
  headers[1] = (uint8_t)(index & 0xffU);
  headers[2] = (uint8_t)((data_size & (uint16_t)0xff0) >> 4);
  headers[3] = (uint8_t)((data_size & (uint16_t)0x00f) << 4);

  VL53L8CX_SwapBuffer(data, data_size);
  for (i = (int16_t)data_size - 1; i >= 0; i--) {
    p_dev->temp_buffer[i + 4] = data[i];
  }

  (void)memcpy(&p_dev->temp_buffer[0], headers, sizeof(headers));
  (void)memcpy(&p_dev->temp_buffer[data_size + 4], footer, sizeof(footer));

  status |= VL53L8CX_WrMulti(&p_dev->platform, address,
                             p_dev->temp_buffer, (uint16_t)(data_size + 12U));
  status |= _vl53l8cx_poll_for_answer(p_dev, 4, 1, VL53L8CX_UI_CMD_STATUS, 0xff, 0x03);
...
uint8_t vl53l8cx_dci_replace_data(VL53L8CX_Configuration *p_dev, uint8_t *data,
